...
namespace Byte8\PingBell\Console\Command;
...
class ActivateCommand extends Command
{
    ...
    {
        $this->setName('byte8:pingbell:activate')
            ->setDescription('Save and verify a Byte8 PingBell activation key.')
            ->addArgument(
                self::ARG_KEY,
                InputArgument::REQUIRED,
                'Activation key from byte8.io/activate/pingbell (format: b8a_pb_...).'
            )
            ->addOption(
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($envelope === null) {
            $output->writeln('<comment>Could not reach api.byte8.io. The key has been saved; the gate will retry on the next notification.</comment>');
            return Command::SUCCESS;
        }

        if (($envelope['ok'] ?? false) === true) {
            $output->writeln('<info>Activation verified successfully.</info>');
            $output->writeln(sprintf('  product: %s', $envelope['productId'] ?? 'pingbell'));
